...
#include "CosUmdCommandBuffer.tmh"
...
CosUmdCommandBuffer::CosUmdCommandBuffer()
{
    m_pCosUmdDevice = NULL;
}
...
void CosUmdCommandBuffer::Standup(CosUmdDevice * pCosUmdDevice, D3DDDICB_CREATECONTEXT * pCreateContext)
{
    m_pCosUmdDevice = pCosUmdDevice;
...
bool CosUmdCommandBuffer::IsCommandBufferEmpty()
{
    return (m_commandBufferPos <= sizeof(GpuCommand));
}

bool CosUmdCommandBuffer::IsSwCommandBuffer()
{
    assert(m_pCmdBufHeader->m_commandId == Header);
    return m_pCmdBufHeader->m_commandBufferHeader.m_swCommandBuffer;
}

// TODO(bhouse) The implementation of copy resource (and write resource below) should not be here.  CosUmdCommandBuffer should
...
void CosUmdCommandBuffer::CopyResource(CosUmdResource * pDstResource, CosUmdResource * pSrcResource)
{
    assert(m_pCosUmdDevice != NULL);
...
void CosUmdCommandBuffer::WriteResource(CosUmdResource * pResource, void * pData)
{
    assert(m_pCosUmdDevice != NULL);
...
void CosUmdCommandBuffer::FlushIfMatching(ULONGLONG fence)
{
    ...
    UINT /*flushFlags*/)
{
    ...
    render.hContext = m_pCosUmdDevice->m_hContext;
...
void
CosUmdCommandBuffer::ReserveCommandBufferSpace(
    bool                        bSwCommand,
    UINT                        commandSize,
...
void
CosUmdCommandBuffer::CommitCommandBufferSpace(
    UINT    commandSize,
    UINT    patchLocationSize)
{
    assert(m_pCosUmdDevice != NULL);
...
UINT
CosUmdCommandBuffer::UseResource(
...
bool
CosUmdCommandBuffer::IsResourceUsed(
    CosUmdResource *    pResource)
{
...
void CosUmdCommandBuffer::UpdateClearColor(
    UINT clearColor)
{
...
void CosUmdCommandBuffer::UpdateClearDepthStencil(
    FLOAT depthValue,
    UINT8 stencilValue)
{
    m_pCmdBufHeader->m_commandBufferHeader.m_hasVC4ClearColors = 1;
