...
CosUmdDevice::CosUmdDevice(
    CosUmdAdapter* pAdapter,
    ...
    m_bPredicateValue(FALSE)
{
    ...
    *m_pWDDM1_3DeviceFuncs = CosUmdDeviceDdi::s_deviceFuncsWDDM1_3;

    assert(IS_DXGI1_3_BASE_FUNCTIONS(pArgs->Interface, pArgs->Version));

    *(pArgs->DXGIBaseDDI.pDXGIDDIBaseFunctions4) = CosUmdDeviceDdi::s_dxgiDeviceFuncs4;
...
void CosUmdDevice::Standup()
{
    ...
    createContext.pPrivateDriverData = &cosContextExchange;
    createContext.PrivateDriverDataSize = sizeof(cosContextExchange);
...
void CosUmdDevice::CreateResource(const D3D11DDIARG_CREATERESOURCE* pCreateResource, D3D10DDI_HRESOURCE hResource, D3D10DDI_HRTRESOURCE hRTResource)
{
    CosUmdResource* pResource = new (hResource.pDrvPrivate) CosUmdResource();
    ...
    if (pCreateResource->BindFlags & D3D10_DDI_BIND_CONSTANT_BUFFER)
    {
        ...
        {
            throw CosUmdException(E_OUTOFMEMORY);
        }
    ...
    {
        CosAllocationExchange* cosAllocationExchange = pResource;
        ...
        auto allocationInfo = D3DDDI_ALLOCATIONINFO{};
        {
            ...
            allocationInfo.pPrivateDriverData = cosAllocationExchange;
            allocationInfo.PrivateDriverDataSize = sizeof(*cosAllocationExchange);
            allocationInfo.VidPnSourceId = 0;
            allocationInfo.Flags.Primary = pCreateResource->pPrimaryDesc != nullptr;
        }

        auto cosAllocationGroupExchange = CosAllocationGroupExchange{};

        allocate.pPrivateDriverData = &cosAllocationGroupExchange;
        allocate.PrivateDriverDataSize = sizeof(cosAllocationGroupExchange);
...
//
// Need to instantiate a new CosUmdResource object in the memory
// pointed to by hResource.pDrvPrivate and initialize it according to
// the existing CosAllocationExchange object pointed to by the pPrivateDriverData
// member of D3DDDI_OPENALLOCATIONINFO.
//
void CosUmdDevice::OpenResource(
    const D3D10DDIARG_OPENRESOURCE* Args,
    D3D10DDI_HRESOURCE hResource,
    D3D10DDI_HRTRESOURCE hRTResource)
{
    assert(Args->PrivateDriverDataSize == sizeof(CosAllocationGroupExchange));
    ...
    auto cosAllocationExchangePtr = static_cast<const CosAllocationExchange*>(
            openAllocationInfoPtr->pPrivateDriverData);

    // Instantiate the new resource in the memory provided to us by the framework
    auto cosUmdResourcePtr = new (hResource.pDrvPrivate) CosUmdResource();

    cosUmdResourcePtr->InitSharedResourceFromExistingAllocation(
            cosAllocationExchangePtr,
            Args->hKMResource,
            openAllocationInfoPtr->hAllocation,
            hRTResource);
}

void CosUmdDevice::DestroyResource(
    CosUmdResource * pResource)
{
    pResource->Teardown();
    pResource->~CosUmdResource();
}

void CosUmdDevice::ResourceCopy(
    CosUmdResource * pDestinationResource,
    CosUmdResource * pSourceResource)
{
...
void CosUmdDevice::ConstantBufferUpdateSubresourceUP(
    CosUmdResource *pDstResource,
    UINT DstSubresource,
...
void CosUmdDevice::CreatePixelShader(
    const UINT* pCode,
    D3D10DDI_HSHADER hShader,
    D3D10DDI_HRTSHADER hRTShader,
    const D3D11_1DDIARG_STAGE_IO_SIGNATURES* pSignatures)
{
    CosUmdPipelineShader* pPipelineShader = new (hShader.pDrvPrivate) CosUmdPipelineShader(this, D3D10_SB_PIXEL_SHADER);

    pPipelineShader->Standup(pCode, hRTShader, pSignatures);
}

void CosUmdDevice::CreateVertexShader(
    const UINT* pCode,
    D3D10DDI_HSHADER hShader,
    D3D10DDI_HRTSHADER hRTShader,
    const D3D11_1DDIARG_STAGE_IO_SIGNATURES* pSignatures)
{
    CosUmdPipelineShader* pPipelineShader = new (hShader.pDrvPrivate) CosUmdPipelineShader(this, D3D10_SB_VERTEX_SHADER);

    pPipelineShader->Standup(pCode, hRTShader, pSignatures);
}

void CosUmdDevice::CreateGeometryShader(
    const UINT* pCode,
    D3D10DDI_HSHADER hShader,
    D3D10DDI_HRTSHADER hRTShader,
    const D3D11_1DDIARG_STAGE_IO_SIGNATURES* pSignatures)
{
    CosUmdPipelineShader* pPipelineShader = new (hShader.pDrvPrivate) CosUmdPipelineShader(this, D3D10_SB_GEOMETRY_SHADER);

    pPipelineShader->Standup(pCode, hRTShader, pSignatures);
}

void CosUmdDevice::CreateComputeShader(
    const UINT* pCode,
    D3D10DDI_HSHADER hShader,
    D3D10DDI_HRTSHADER hRTShader)
{
    CosUmdPipelineShader* pPipelineShader = new (hShader.pDrvPrivate) CosUmdPipelineShader(this, D3D11_SB_COMPUTE_SHADER);

    pPipelineShader->Standup(pCode, hRTShader, NULL);
}

void CosUmdDevice::CreateTessellationShader(
    const UINT * pCode,
    D3D10DDI_HSHADER hShader,
    D3D10DDI_HRTSHADER hRTShader,
...
{
    CosUmdTesselationShader* pTessellationShader = new (hShader.pDrvPrivate) CosUmdTesselationShader(this, ProgramType);

    pTessellationShader->Standup(pCode, hRTShader, pSignatures);
}

void CosUmdDevice::DestroyShader(
    D3D10DDI_HSHADER hShader)
{
    CosUmdShader * pShader = CosUmdShader::CastFrom(hShader);

    pShader->Teardown();
    pShader->~CosUmdShader();
}

void CosUmdDevice::ResourceMap(
    CosUmdResource * pResource,
    UINT subResource,
...
void CosUmdDevice::ResourceUnmap(
    CosUmdResource * pResource,
    UINT subResource)
{
...
void CosUmdDevice::CheckFormatSupport(
...
void CosUmdDevice::CheckCounterInfo(
...
void CosUmdDevice::CheckMultisampleQualityLevels(
    DXGI_FORMAT inFormat,
    UINT inSampleCount,
    UINT inFlags,
...
void CosUmdDevice::Lock(D3DDDICB_LOCK * pLock)
{
    ...
    if (hr != S_OK) throw CosUmdException(hr);
}

void CosUmdDevice::Unlock(D3DDDICB_UNLOCK * pUnlock)
{
...
void CosUmdDevice::Render(D3DDDICB_RENDER * pRender)
{
...
void CosUmdDevice::DestroyContext(D3DDDICB_DESTROYCONTEXT * pDestroyContext)
{
...
HRESULT CosUmdDevice::Present(DXGI_DDI_ARG_PRESENT* Args)
{
    ...
    auto pSrcResource = CosUmdResource::CastFrom(Args->hSurfaceToPresent);
    ...
    {
        auto pDstResource = CosUmdResource::CastFrom(Args->hDstResource);
...
HRESULT CosUmdDevice::RotateResourceIdentities(
    DXGI_DDI_ARG_ROTATE_RESOURCE_IDENTITIES* Args)
{
    assert(CosUmdDevice::CastFrom(Args->hDevice) == this);

    assert(Args->Resources != 0);

    // Save the handles from the first resource
    const CosUmdResource* const firstResource = CosUmdResource::CastFrom(
            Args->pResources[0]);
    CosUmdResource* const lastResource = CosUmdResource::CastFrom(
    ...
    {
        CosUmdResource* rotateTo = CosUmdResource::CastFrom(Args->pResources[i]);
        const CosUmdResource* rotateFrom = CosUmdResource::CastFrom(
                Args->pResources[i + 1]);
...
HRESULT CosUmdDevice::SetDisplayMode(DXGI_DDI_ARG_SETDISPLAYMODE* Args)
{
    assert(CosUmdDevice::CastFrom(Args->hDevice) == this);

    // Translate hResource and SubResourceIndex to a primary allocation handle
    auto pResource = reinterpret_cast<CosUmdResource*>(Args->hResource);
...
HRESULT CosUmdDevice::Present1(DXGI_DDI_ARG_PRESENT1* Args)
{
    assert(this == CastFrom(Args->hDevice));
    ...
    {
        // Get the allocation for the source resource
        auto pSrcResource = CosUmdResource::CastFrom(
        ...
        if (Args->hDstResource)
        {
            auto pDstResource = CosUmdResource::CastFrom(
                    Args->hDstResource);
...
void CosUmdDevice::CheckDirectFlipSupport(
    ...
    )
{
    ...
    //
    const CosUmdResource* currentResourcePtr = CosUmdResource::CastFrom(hResource1);
    const CosUmdResource* directFlipResourcePtr = CosUmdResource::CastFrom(hResource2);
...
void CosUmdDevice::SetError(HRESULT hr)
{
...
void CosUmdDevice::SetException(const CosUmdException & e)
{
    SetError(e.m_hr);
}

void CosUmdDevice::SetException(const std::exception & e)
{
    auto cosUmdException = dynamic_cast<const CosUmdException*>(&e);
    if (cosUmdException)
    {
        SetError(cosUmdException->m_hr);
    }
    else
    {
        SetError(E_FAIL);
...
void CosUmdDevice::Draw(UINT vertexCount, UINT startVertexLocation)
{
    //
...
void CosUmdDevice::DrawIndexed(UINT indexCount, UINT startIndexLocation, INT baseVertexLocation)
{
    //
...
void CosUmdDevice::ClearRenderTargetView(CosUmdRenderTargetView * pRenderTargetView, FLOAT clearColor[4])
{
#if !VC4
    // do nothing
    pRenderTargetView;
    ...
    CosUmdResource * pRenderTarget = CosUmdResource::CastFrom(pRenderTargetView->m_create.hDrvResource);
...
void CosUmdDevice::ClearDepthStencilView(CosUmdDepthStencilView * pDepthStencilView, UINT clearFlags, FLOAT depthValue, UINT8 stencilValue)
{
...
void CosUmdDevice::SetVertexBuffers(UINT startBuffer, UINT numBuffers, const D3D10DDI_HRESOURCE* phBuffers, const UINT* pStrides, const UINT* pOffsets)
{
    assert(startBuffer + numBuffers <= kMaxVertexBuffers);

    for (UINT i = 0; i < numBuffers; i++)
    {
        m_vertexBuffers[i+startBuffer] = CosUmdResource::CastFrom(phBuffers[i]);
    }
...
void CosUmdDevice::SetIndexBuffer(const D3D10DDI_HRESOURCE indexBuffer, DXGI_FORMAT indexFormat, UINT offset)
{
    m_indexBuffer = CosUmdResource::CastFrom(indexBuffer);
    m_indexFormat = indexFormat;
    m_indexOffset = offset;
}

void CosUmdDevice::VsSetConstantBuffers11_1(
    UINT startBuffer,
    UINT numberBuffers,
    ...
    const UINT *    pNumberConstants)
{
    ...
    {
        m_vsConstantBuffer[bufIndex] = CosUmdResource::CastFrom(phResources[i]);
    }
...
void CosUmdDevice::SetTopology(D3D10_DDI_PRIMITIVE_TOPOLOGY topology)
{
    m_topology = topology;
}

void CosUmdDevice::SetViewports(UINT numViewports, UINT clearViewports, const D3D10_DDI_VIEWPORT* pViewports)
{
...
void CosUmdDevice::SetRenderTargets(const D3D10DDI_HRENDERTARGETVIEW* phRenderTargetView, UINT numRTVs, UINT rtvNumbertoUnbind,
    ...
    const UINT* pUAVInitialCounts, UINT UAVIndex, UINT numUAVs, UINT UAVFirsttoSet, UINT UAVNumberUpdated)
{
    ...
    {
        m_renderTargetViews[i] = CosUmdRenderTargetView::CastFrom(phRenderTargetView[i]);
    }
    m_numRenderTargetViews = numRTVs;

    m_depthStencilView = CosUmdDepthStencilView::CastFrom(hDepthStencilView);
}

void CosUmdDevice::SetBlendState(CosUmdBlendState * pBlendState, const FLOAT pBlendFactor[4], UINT sampleMask)
{
...
void CosUmdDevice::SetPixelShader(CosUmdShader * pShader)
{
    m_pixelShader = pShader;
}

void CosUmdDevice::SetPixelSamplers(UINT Offset, UINT NumSamplers, const D3D10DDI_HSAMPLER* phSamplers)
{
    assert(Offset + NumSamplers <= kMaxSamplers);
    for (UINT i = 0; i < NumSamplers; i++)
    {
        m_pixelSamplers[i+Offset] = CosUmdSampler::CastFrom(phSamplers[i]);
    }
}

void CosUmdDevice::PSSetShaderResources(UINT offset, UINT numViews, const D3D10DDI_HSHADERRESOURCEVIEW * phShaderResourceViews)
{
    for (UINT i = 0; i < numViews; i++)
    {
        m_psResourceViews[offset + i] = CosUmdShaderResourceView::CastFrom(phShaderResourceViews[i]);
    }
}

void CosUmdDevice::PsSetConstantBuffers11_1(
    UINT startBuffer,
    UINT numberBuffers,
    const D3D10DDI_HRESOURCE *  phResources,
    const UINT *    pFirstConstant,
    const UINT *    pNumberConstants)
{
    ...
    for (UINT i = 0; i < numberBuffers; i++, bufIndex++)
    {
        m_psConstantBuffer[bufIndex] = CosUmdResource::CastFrom(phResources[i]);
    }
...
void CosUmdDevice::SetVertexShader(CosUmdShader * pShader)
{
    m_vertexShader = pShader;
}

void CosUmdDevice::SetVertexSamplers(UINT Offset, UINT NumSamplers, const D3D10DDI_HSAMPLER* phSamplers)
{
    assert(Offset + NumSamplers <= kMaxSamplers);
    for (UINT i = 0; i < NumSamplers; i++)
    {
        m_vertexSamplers[i+Offset] = CosUmdSampler::CastFrom(phSamplers[i]);
    }
}

void CosUmdDevice::SetDomainShader(CosUmdShader * pShader)
{
    m_domainShader = pShader;
}

void CosUmdDevice::SetDomainSamplers(UINT Offset, UINT NumSamplers, const D3D10DDI_HSAMPLER* phSamplers)
{
    assert(Offset + NumSamplers <= kMaxSamplers);
    for (UINT i = 0; i < NumSamplers; i++)
    {
        m_domainSamplers[i+ Offset] = CosUmdSampler::CastFrom(phSamplers[i]);
    }
}

void CosUmdDevice::SetGeometryShader(CosUmdShader * pShader)
{
    m_geometryShader = pShader;
}

void CosUmdDevice::SetGeometrySamplers(UINT Offset, UINT NumSamplers, const D3D10DDI_HSAMPLER* phSamplers)
{
    assert(Offset + NumSamplers <= kMaxSamplers);
    for (UINT i = 0; i < NumSamplers; i++)
    {
        m_geometrySamplers[i] = CosUmdSampler::CastFrom(phSamplers[i]);
    }
}

void CosUmdDevice::SetHullShader(CosUmdShader * pShader)
{
    m_hullShader = pShader;
}

void CosUmdDevice::SetHullSamplers(UINT Offset, UINT NumSamplers, const D3D10DDI_HSAMPLER* phSamplers)
{
    assert(Offset + NumSamplers <= kMaxSamplers);
    for (UINT i = 0; i < NumSamplers; i++)
    {
        m_hullSamplers[i] = CosUmdSampler::CastFrom(phSamplers[i]);
    }
}

void CosUmdDevice::SetComputeShader(CosUmdShader * pShader)
{
    m_computeShader = pShader;
}

void CosUmdDevice::SetComputeSamplers(UINT Offset, UINT NumSamplers, const D3D10DDI_HSAMPLER* phSamplers)
{
    assert(Offset + NumSamplers <= kMaxSamplers);
    for (UINT i = 0; i < NumSamplers; i++)
    {
...
void CosUmdDevice::SetDepthStencilState(CosUmdDepthStencilState * pDepthStencilState, UINT stencilRef)
{
    m_depthStencilState = pDepthStencilState;
    m_stencilRef = stencilRef;
}

void CosUmdDevice::SetRasterizerState(CosUmdRasterizerState * pRasterizerState)
{
    m_rasterizerState = pRasterizerState;
}

void CosUmdDevice::SetScissorRects(UINT NumScissorRects, UINT ClearScissorRects, const D3D10_DDI_RECT *pRects)
{
    NumScissorRects;
    ClearScissorRects;
    pRects;
...
void CosUmdDevice::RefreshPipelineState(UINT vertexOffset)
{
    CosUmdResource * pRenderTarget = CosUmdResource::CastFrom(m_renderTargetViews[0]->m_create.hDrvResource);
    ...
    pPSUniformEntries = m_pixelShader->GetShaderUniformFormat(COS_PIXEL_SHADER_UNIFORM_STORAGE, &numPSUniformEntries);
    pVSUniformEntries = m_vertexShader->GetShaderUniformFormat(COS_VERTEX_SHADER_UNIFORM_STORAGE, &numVSUniformEntries);
    pCSUniformEntries = m_vertexShader->GetShaderUniformFormat(COS_COORDINATE_SHADER_UNIFORM_STORAGE, &numCSUniformEntries);
...
void CosUmdDevice::WriteUniforms(
    BOOLEAN                     bPSUniform,
        ...
        case VC4_UNIFORM_TYPE_USER_CONSTANT:
            {
                ...
                CosUmdResource *    pConstantBuffer;
        ...
        case VC4_UNIFORM_TYPE_SAMPLER_CONFIG_P0:
            {
                ...
                pTexture = CosUmdResource::CastFrom(m_psResourceViews[pCurUniformEntry->samplerConfiguration.resourceIndex]->m_create.hDrvResource);
        ...
        case VC4_UNIFORM_TYPE_SAMPLER_CONFIG_P1:
            {
                ...
                if (pTexture->m_hwLayout == CosHwLayout::Tiled)
                {
                ...
                CosUmdSampler * pSampler = m_pixelSamplers[pCurUniformEntry->samplerConfiguration.samplerIndex];
...
void CosUmdDevice::CreateInternalBuffer(CosUmdResource * pRes, UINT size)
{
...
void CosUmdDevice::SetPredication(D3D10DDI_HQUERY hQuery, BOOL bPredicateValue)
{
    //
...
void CosUmdDevice::ResourceCopyRegion11_1(
    CosUmdResource *pDestinationResource,
    UINT DstSubresource,
    UINT DstX,
    UINT DstY,
