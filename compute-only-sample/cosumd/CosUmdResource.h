...
class CosUmdResource : public CosAllocationExchange
{
...
public:
    ...
    static CosUmdResource* CastFrom(D3D10DDI_HRESOURCE);
    static CosUmdResource* CastFrom(DXGI_DDI_HRESOURCE);
    ...
    bool CanRotateFrom(const CosUmdResource* Other) const;
    ...
    UINT WidthInTiles () const
    {
        ...
        NT_ASSERT(m_hwLayout == CosHwLayout::Tiled);
...
inline CosUmdResource* CosUmdResource::CastFrom(D3D10DDI_HRESOURCE hResource)
{
    auto thisPtr = static_cast< CosUmdResource* >(hResource.pDrvPrivate);
    if (thisPtr) { assert(thisPtr->m_signature == _SIGNATURE::INITIALIZED); }
    return thisPtr;
}

inline CosUmdResource* CosUmdResource::CastFrom(DXGI_DDI_HRESOURCE hResource)
{
    static_assert(
        sizeof(hResource) == sizeof(CosUmdResource*),
        "Sanity check on cast compatibility");
    auto thisPtr = reinterpret_cast<CosUmdResource*>(hResource);
    if (thisPtr) { assert(thisPtr->m_signature == _SIGNATURE::INITIALIZED); }
    return thisPtr;
}

inline D3D10DDI_HRESOURCE CosUmdResource::CastTo() const
{
