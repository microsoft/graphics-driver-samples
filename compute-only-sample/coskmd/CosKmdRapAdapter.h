...
class CosKmdRapAdapter : public CosKmAdapter
{
private:

    friend class CosKmAdapter;

    CosKmdRapAdapter(IN_CONST_PDEVICE_OBJECT PhysicalDeviceObject, OUT_PPVOID MiniportDeviceContext);
    virtual ~CosKmdRapAdapter();
...
private:
    ...
    UINT GenerateRenderingControlList(COSDMABUFINFO *pDmaBufInf);
    ...
    void MoveToNextBinnerRenderMemChunk(UINT controlListLength)
    {
        ...
        if ((m_tileStateDataArrayPhysicalAddress + 64 * kPageSize) >= (CosKmdGlobal::s_videoMemoryPhysicalAddress.LowPart + CosKmdGlobal::s_videoMemorySize))
        {
