    ...
    IN_POWER_ACTION         ActionType)
{
    ...
    {
        COS_LOG_WARNING("SetPowerState() is not implemented by render side.");
...
NTSTATUS
CosKmAdapter::PowerRuntimeControlRequest(
...
NTSTATUS
CosKmAdapter::InitializePowerComponentInfo()
{
    NTSTATUS Status;
        ...
        (acpiReader.GetOutputArgumentCount() == 3)) // must return 3 output arguments
    {
        ...
        CosKmAcpiArgumentParser acpiPowerComponentParser(&acpiReader, pPowerComponentPackage);
        for (ULONG i = 0; i < numPowerCompoment; i++)
        {
            ...
            CosKmAcpiArgumentParser acpiComponentParser(&acpiReader, pComponentPackage);
            ...
            CosKmAcpiArgumentParser acpiPowerStateParser(&acpiReader, pPowerStatePackage);
            for (ULONG j = 0; j < m_PowerComponents[componentIndex].StateCount; j++)
            {
                ...
                CosKmAcpiArgumentParser acpiStateParser(&acpiReader, pStatePackage);
...
NTSTATUS
CosKmAdapter::GetNumPowerComponents()
{
    return m_NumPowerComponents;
}

NTSTATUS
CosKmAdapter::GetPowerComponentInfo(
...
NTSTATUS
CosKmAdapter::SetPowerComponentFState(
    IN UINT ComponentIndex,
    IN UINT FState)
{
    //
