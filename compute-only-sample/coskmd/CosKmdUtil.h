#ifndef _COSKMDUTIL_H_
#define _COSKMDUTIL_H_ 1
...
// Abstract:
//
//    This is CosKmd shared utilities include file
//
// Author:
//
...
#define COS_NONPAGED_SEGMENT_BEGIN \
    __pragma(code_seg(push)) \
    //__pragma(code_seg(.text))

#define COS_NONPAGED_SEGMENT_END \
    __pragma(code_seg(pop))

#define COS_PAGED_SEGMENT_BEGIN \
    __pragma(code_seg(push)) \
    __pragma(code_seg("PAGE"))

#define COS_PAGED_SEGMENT_END \
    __pragma(code_seg(pop))

#define COS_INIT_SEGMENT_BEGIN \
    __pragma(code_seg(push)) \
    __pragma(code_seg("INIT"))

#define COS_INIT_SEGMENT_END \
    __pragma(code_seg(pop))
...
//
#define COS_ASSERT_MAX_IRQL(Irql) NT_ASSERT(KeGetCurrentIrql() <= (Irql))
#define COS_ASSERT_LOW_IRQL() COS_ASSERT_MAX_IRQL(APC_LEVEL)

//
// Pool allocation tags for use by CosKmd
//
enum COS_ALLOC_TAG : ULONG {
...
__forceinline NTSTATUS CosSanitizeNtstatus (
    NTSTATUS Status,
...
NTSTATUS CosOpenDevice (
    ...
    COS_ALLOC_TAG AllocTag = COS_ALLOC_TAG::DEVICE
    );
