...
void VC4_DISPLAY::ResetDevice ()
{
    ...
    COS_LOG_ERROR("Vc4DdiResetDevice() was called but is not implemented.");
}
...
{
    COS_LOG_ASSERTION("Not implemented");
    ...
    )
{
    ...
    const auto cosKmdAllocation =
            static_cast<CosKmdAllocation*>(Args->hAllocation);
    ...
    if (Args->Flags.ModeChange) {
        ...
        NT_ASSERT(cosKmdAllocation->m_isPrimary);
        const DXGI_DDI_MODE_DESC* desiredModeDescPtr =
                &cosKmdAllocation->m_primaryDesc.ModeDesc;
    ...
    if (Args->Flags.SharedPrimaryTransition) {
        COS_LOG_WARNING("What do we do here?");
    }

    if (Args->Flags.IndependentFlipExclusive) {
        COS_LOG_ASSERTION("What do we do here?");
    }

    NT_ASSERT(
        (CosKmdGlobal::s_videoMemoryPhysicalAddress.HighPart == 0) &&
        (Args->PrimaryAddress.HighPart == 0));

    // Verify memory is in range
    NT_ASSERT(Args->PrimarySegment == COSD_SEGMENT_VIDEO_MEMORY);
    NT_ASSERT(Args->PrimaryAddress.LowPart < CosKmdGlobal::s_videoMemorySize);
    if (cosKmdAllocation) {
        NT_ASSERT(
            (Args->PrimaryAddress.LowPart + cosKmdAllocation->m_hwSizeBytes) <=
            CosKmdGlobal::s_videoMemorySize);
    }
    ...
    COS_TRACE_EVENTS(
        ...
        Args->PrimaryAddress.LowPart,
        physicAddress,
        static_cast<const BYTE*>(CosKmdGlobal::s_pVideoMemory) + Args->PrimaryAddress.LowPart);
    return STATUS_SUCCESS;
}
    ...
    )
{
    ...
    // VSync interrupt
    if (intStat.VfpStart) {
        // COS_LOG_TRACE("Notifying dxgkrnl of VSYNC interrupt.");
        ...
        COS_LOG_ASSERTION("Unexpected interrupt!");
    }
    ...
    )
{
    ...
    auto unmapHvsRegisters = COS_FINALLY::DoUnless([&] {
        PAGED_CODE();
    ...
    auto unmapPvRegisters = COS_FINALLY::DoUnless([&] {
        PAGED_CODE();
        NTSTATUS unmapStatus = this->dxgkInterface.DxgkCbUnmapMemory(
                this->dxgkInterface.DeviceHandle,
                _pvRegistersPtr);
    ...
    // Ensure HVS register block is enabled
    {
        ...
        if (!dispCtrl.ENABLE) {
            COS_LOG_ERROR("The HVS is not enabled");
    ...
    auto unmapBiosFrameBuffer = COS_FINALLY::DoUnless([&] {
        PAGED_CODE();
    ...
    // Validate and store a pointer to the active display list
    {
        ...
        if ((dispList1.HEADE + ARRAYSIZE(displayListCopy.AsUlong)) >
            ...
            COS_LOG_ASSERTION("Display list address is out of range");
            return STATUS_INVALID_DEVICE_STATE;
        }
        ...
        if (displayListCopy.PointerWord0 !=
            ...
            COS_LOG_ASSERTION("The display list does not point to the BIOS buffer");
            return STATUS_INVALID_DEVICE_STATE;
        }
    ...
    *NumberOfChildrenPtr = CHILD_COUNT;     // represents the HDMI connector
    ...
    COS_LOG_TRACE("Successfully started device.");
    return STATUS_SUCCESS;
}
...
void VC4_DISPLAY::StopDevice ()
{
    ...
    COS_LOG_TRACE("Successfully stopped device.");
}
    ...
    )
{
    ...
    COS_LOG_TRACE("Child relations reported successfully.");
    return STATUS_SUCCESS;
}
    ...
    )
{
    ...
    case StatusConnection:
        ...
        break;
    case StatusRotation:
        COS_LOG_ERROR("Received StatusRotation query even though D3DKMDT_MOA_NONE was reported.");
        return STATUS_INVALID_PARAMETER;
    case StatusMiracastConnection:
        COS_LOG_ERROR("Miracast is not supported.");
        return STATUS_NOT_SUPPORTED;
    ...
    )
{
    ...
    case DISPLAY_ADAPTER_HW_ID:
        ...
        break;
    default:
        COS_LOG_ASSERTION("Invalid DeviceUid. (DeviceUid = %d)", DeviceUid);
        return STATUS_INVALID_PARAMETER;
    }
    ...
    )
{
    ...
    if (!SetPointerPositionPtr->Flags.Visible) {
        COS_LOG_TRACE("Received request to set pointer visibility to OFF.");
        return STATUS_SUCCESS;
    } else {
        COS_LOG_ASSERTION("SetPointerPosition should never be called to set the pointer to visible since VC4 doesn't support hardware cursors.");
        return STATUS_UNSUCCESSFUL;
    }
}
    ...
    )
{
    ...
    COS_LOG_ASSERTION("SetPointerShape should never be called since VC4 doesn't support hardware cursors.");
    return STATUS_NOT_IMPLEMENTED;
}
    ...
    )
{
    ...
    auto releasePathInfo = COS_FINALLY::Do([&, presentPathPtr] () {
        PAGED_CODE();
    ...
    )
{
    ...
    auto releaseSms = COS_FINALLY::Do([&] () {
        PAGED_CODE();
    ...
    auto releaseModeInfo = COS_FINALLY::Do([&] () {
        PAGED_CODE();
    ...
    for (SIZE_T pathIndex = 0; pathIndex < numPathsFromSource; ++pathIndex) {
        ...
        auto releasePathInfo = COS_FINALLY::Do([&] {
            PAGED_CODE();
            NTSTATUS releaseStatus = topologyInterfacePtr->pfnReleasePathInfo(
                    vidPnTopologyHandle,
                    presentPathPtr);
    ...
    )
{
    ...
    auto releaseMonitorMode = COS_FINALLY::DoUnless([&] () {
        PAGED_CODE();
    ...
    )
{
    ...
    case DXGK_INTERRUPT_CRTC_VSYNC:
        if (EnableInterrupt) {
            COS_LOG_TRACE("Enabling CRTC_VSYNC interrupt");
            ...
            COS_LOG_TRACE("Disabling CRTC_VSYNC interrupt");
    ...
    )
{
    ...
    if (!NT_SUCCESS(status)) {
        COS_TRACE_EVENTS(
            ...
            "DXGK_VIDPN_INTERFACE::pfnAcquireSourceModeSet() failed. (status = %!STATUS!)",
            status);
        return CosSanitizeNtstatus(status);
    }
    NT_ASSERT(sourceModeSetHandle);
    auto releaseSms = COS_FINALLY::Do([&] () {
        PAGED_CODE();
    ...
    ) const
{
    ...
    NT_ASSERT(sourceModeSetHandle);
    auto releaseSms = COS_FINALLY::DoUnless([&] () {
        PAGED_CODE();
        NTSTATUS releaseStatus = VidPnInterfacePtr->pfnReleaseSourceModeSet(
                VidPnHandle,
                sourceModeSetHandle);
    ...
    // Create new mode info and add it to the source mode set
    {
        ...
        auto releaseModeInfo = COS_FINALLY::DoUnless([&] () {
            PAGED_CODE();
            NTSTATUS releaseStatus = smsInterfacePtr->pfnReleaseModeInfo(
                    sourceModeSetHandle,
                    sourceModeInfoPtr);
    ...
    )
{
    ...
    auto releaseTargetModeSet = COS_FINALLY::Do([&] () {
        PAGED_CODE();
