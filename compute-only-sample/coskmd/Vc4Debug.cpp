    ...
    )
{
    ...
    while (status == STATUS_SUCCESS) {
        NT_ASSERT(modeInfo);
        auto releaseModeInfo = COS_FINALLY::Do([&, modeInfo] () {
            PAGED_CODE();
            NTSTATUS releaseStatus =
    ...
    )
{
    ...
    auto releaseTms = COS_FINALLY::Do([&] () {
        PAGED_CODE();
        NTSTATUS releaseStatus =
