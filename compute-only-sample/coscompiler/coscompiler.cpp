...
CosCompiler* CosCompilerCreate(D3D10_SB_TOKENIZED_PROGRAM_TYPE ProgramType,
                               ...
                               const CosUmdRenderTargetView** ppRenderTargetView,
                               const CosUmdShaderResourceView** ppShaderResouceView,
...
{
    CosCompiler *pCompiler = new CosCompiler(
        ProgramType,
        pCode,
...
CosCompiler::CosCompiler(D3D10_SB_TOKENIZED_PROGRAM_TYPE ProgramType,
                         const UINT *pCode,
                         const UINT *pLinkageDownstreamCode,
                         const UINT *pLinkageUpstreamCode,
                         const D3D11_1_DDI_BLEND_DESC* pBlendState,
...
CosCompiler::~CosCompiler() 
{
}

HRESULT CosCompiler::Compile()
{
    ...
    case D3D10_SB_VERTEX_SHADER:
        ...
        Vc4ShaderCompiler.SetShaderStorage(
            &m_Storage[COS_VERTEX_SHADER_STORAGE],
            &m_Storage[COS_VERTEX_SHADER_UNIFORM_STORAGE]);
        // for coordinate shader
        Vc4ShaderCompiler.SetShaderStorageAux(
            &m_Storage[COS_COORDINATE_SHADER_STORAGE],
            &m_Storage[COS_COORDINATE_SHADER_UNIFORM_STORAGE]);
        ...
        if (SUCCEEDED(hr))
        {
            ...
            Disassemble_HW(m_Storage[COS_VERTEX_SHADER_STORAGE], TEXT("VC4 Vertex shader"));
            Dump_UniformTable(m_Storage[COS_VERTEX_SHADER_UNIFORM_STORAGE], TEXT("VC4 Vertex shader Uniform"));

            Disassemble_HW(m_Storage[COS_COORDINATE_SHADER_STORAGE], TEXT("VC4 Coordinate shader"));
            Dump_UniformTable(m_Storage[COS_COORDINATE_SHADER_UNIFORM_STORAGE], TEXT("VC4 Coordinate shader Uniform"));
#endif // DBG
        }
    ...
    case D3D10_SB_PIXEL_SHADER:
        // Set output hw shader storage.
        Vc4ShaderCompiler.SetShaderStorage(
            &m_Storage[COS_PIXEL_SHADER_STORAGE],
            &m_Storage[COS_PIXEL_SHADER_UNIFORM_STORAGE]);;
...
#if DBG
            // Disassemble h/w shader.
            Disassemble_HW(m_Storage[COS_PIXEL_SHADER_STORAGE], TEXT("VC4 Pixel shader"));
            Dump_UniformTable(m_Storage[COS_PIXEL_SHADER_UNIFORM_STORAGE], TEXT("VC4 Vertex shader Uniform"));
#endif // DBG
