...
public:
    ...
    {
        VC4_THROW(this->Ensure(Storage.GetUsedSize())); // throw CosCompilerException on failure.
    ...
    void Store(_Ty p)
    {
        VC4_THROW(this->Ensure(sizeof(_Ty))); // throw CosCompilerException on failure.
...
public:
    ...
    Vc4Shader(CosCompiler *Compiler) :
...
#define COS_VC4_INPUT_REGISTER_FILE         VC4_QPU_ALU_REG_A
#define COS_VC4_INPUT_REGISTER_FILE_START   0
#define COS_VC4_INPUT_REGISTER_FILE_END     14
    // VC4 VPM limitation, max 15 float(s) input.
    C_ASSERT((COS_VC4_INPUT_REGISTER_FILE_END - COS_VC4_INPUT_REGISTER_FILE_START + 1) < 16);
    // ra15        : Reserved - W (in pixel shader only)
    // ra16 ~ ra31 : Temp (4x4, up to 4 temps)
#define COS_VC4_TEMP_REGISTER_FILE          VC4_QPU_ALU_REG_A
#define COS_VC4_TEMP_REGISTER_FILE_START    16
#define COS_VC4_TEMP_REGISTER_FILE_END      31
    C_ASSERT((COS_VC4_TEMP_REGISTER_FILE_END - COS_VC4_TEMP_REGISTER_FILE_START + 1) == (4*4));
    // rb16 ~ rb31 : Scratch (15 floats)
#define COS_VC4_SCRATCH_REGISTER_FILE       VC4_QPU_ALU_REG_B
#define COS_VC4_SCRATCH_REGISTER_FILE_START 0
#define COS_VC4_SCRATCH_REGISTER_FILE_END   14
    // rb15        : Reserved - Z (in pixel shader only)
    // rb0 ~ rb14  : Output (up to 16 floats)
#define COS_VC4_OUTPUT_REGISTER_FILE        VC4_QPU_ALU_REG_B
#define COS_VC4_OUTPUT_REGISTER_FILE_START  16
#define COS_VC4_OUTPUT_REGISTER_FILE_END    31 
};
