...
class CosUmdDevice;
#include "..\cosumd\CosUmdResource.h"
#include "..\cosumd\CosUmdRenderTargetView.h"
#include "..\cosumd\CosUmdShaderResourceView.h"
...
//
#define COS_VERTEX_SHADER_STORAGE 0
#define COS_VERTEX_SHADER_UNIFORM_STORAGE 1
#define COS_COORDINATE_SHADER_STORAGE  2
#define COS_COORDINATE_SHADER_UNIFORM_STORAGE  3

#define COS_PIXEL_SHADER_STORAGE 0
#define COS_PIXEL_SHADER_UNIFORM_STORAGE 1
...
public:
    ...
    CosCompiler(
        ...
        const CosUmdRenderTargetView** m_ppRenderTargetView,
        const CosUmdShaderResourceView** m_ppShaderResouceView,
    ...
    HRESULT GetShaderCode(void *pDest, UINT* pCSOffset = NULL)
    {
        ...
        case D3D10_SB_VERTEX_SHADER:
            memcpy(pDest, 
                   m_Storage[COS_VERTEX_SHADER_STORAGE].GetStorage(),
                   m_Storage[COS_VERTEX_SHADER_STORAGE].GetUsedSize());
            memcpy((BYTE*)pDest + m_Storage[COS_VERTEX_SHADER_STORAGE].GetUsedSize(),
                   m_Storage[COS_COORDINATE_SHADER_STORAGE].GetStorage(),
                   m_Storage[COS_COORDINATE_SHADER_STORAGE].GetUsedSize());
            assert(pCSOffset);
            *pCSOffset = m_Storage[COS_VERTEX_SHADER_STORAGE].GetUsedSize();
            hr = S_OK;
            break;
        case D3D10_SB_PIXEL_SHADER:
            memcpy(pDest,
                m_Storage[COS_PIXEL_SHADER_STORAGE].GetStorage(),
                m_Storage[COS_PIXEL_SHADER_STORAGE].GetUsedSize());
            hr = S_OK;
            break;
        default:
            assert(false);
    ...
    UINT GetShaderCodeSize()
    {
        ...
        {
        case D3D10_SB_VERTEX_SHADER:
            CodeSize = m_Storage[COS_VERTEX_SHADER_STORAGE].GetUsedSize() +
                       m_Storage[COS_COORDINATE_SHADER_STORAGE].GetUsedSize();
            break;
        case D3D10_SB_PIXEL_SHADER:
            CodeSize = m_Storage[COS_PIXEL_SHADER_STORAGE].GetUsedSize();
            break;
        default:
            assert(false);
        }
    ...
    {
        assert((Type == COS_VERTEX_SHADER_UNIFORM_STORAGE) ||
            (Type == COS_COORDINATE_SHADER_UNIFORM_STORAGE) ||
            (Type == COS_PIXEL_SHADER_UNIFORM_STORAGE));
    ...
    {
        CosUmdResource *pResource = CosUmdResource::CastFrom(m_ppRenderTargetView[i]->m_create.hDrvResource);
        return pResource->m_format;
    }

    const DXGI_FORMAT GetShaderResourceFormat(uint8_t i)
    {
        CosUmdResource *pResource = CosUmdResource::CastFrom(m_ppShaderResouceView[i]->m_create.hDrvResource);
        return pResource->m_format;
    }
...
private:
    ...
    // Resource binding
    //
    const CosUmdRenderTargetView** m_ppRenderTargetView; // point array, size of D3D11_SIMULTANEOUS_RENDER_TARGET_COUNT.
    const CosUmdShaderResourceView** m_ppShaderResouceView; // point array, size of  D3D11_COMMONSHADER_INPUT_RESOURCE_SLOT_COUNT.
...
CosCompiler* CosCompilerCreate(D3D10_SB_TOKENIZED_PROGRAM_TYPE ProgramType,
                               const UINT *pCode,
                               const UINT *pLinkageDownstreamCode,
                               const UINT *pLinkageUpstreamCode,
                               const D3D11_1_DDI_BLEND_DESC* pBlendState,
